...
type VoiceStep = { id: string; step_number: number; action: string; status: string }
type VisualReference = { id: string; title: string; source_url: string; image_url?: string | null; publisher?: string | null }
type Props = { projectId: string; caseId: string; caseTitle: string; stage: string; onChanged?: () => void }
...
export function VoiceAssistant({ projectId, caseId, caseTitle, stage, onChanged }: Props) {
  ...
  async function executeTool(name: string, args: Record<string, any>) {
    setStatus("working"); setMessage(`Updating ${name.replace(/_/g, " ")}…`)
    ...
    if (name === "record_step_result") {
      ...
      const result = await request(`/projects/${projectId}/test-cases/${caseId}/steps/${args.step_id}/executions/${execution.id}`, { method: "PATCH", body: JSON.stringify({ status: args.status, observed_result: args.observed_result, deviation_reference: args.deviation_reference || null }) }); onChanged?.(); return result
    }
  ...
  async function start() {
    ...
    try {
      const steps: VoiceStep[] = await request(`/projects/${projectId}/test-cases/${caseId}/steps`); const pc = new RTCPeerConnection(); pcRef.current = pc
      const audio = new Audio(); audio.autoplay = true; pc.ontrack = (event) => { audio.srcObject = event.streams[0] }
      ...
      const dc = pc.createDataChannel("oai-events"); dcRef.current = dc; dc.onmessage = (event) => { void handleEvent(JSON.parse(event.data)) }; dc.onopen = () => { setStatus("connected"); setMessage("Listening") }
      const offer = await pc.createOffer(); await pc.setLocalDescription(offer); const token = localStorage.getItem("access_token"); const context = JSON.stringify({ project_id: projectId, case_id: caseId, case_title: caseTitle, stage, steps })
      const response = await fetch("/api/v1/realtime/session", { method: "POST", body: offer.sdp, headers: { "Content-Type": "application/sdp", "X-Voice-Context": context, ...(token ? { Authorization: `Bearer ${token}` } : {}) } })
  ...
  return <div className="grid gap-2"><div className="flex items-center gap-2"><button type="button" onClick={active ? stop : () => void start()} className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs ${active ? "border-emerald-500/50 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "border-border hover:bg-muted"}`} aria-label={active ? "Stop voice assistant" : "Start voice assistant"}>{active ? <Square className="size-3.5" /> : <Mic className="size-3.5" />}{status === "connecting" ? "Connecting" : status === "working" ? "Updating" : active ? "Listening" : "Voice"}</button>{message && <span className="max-w-32 truncate text-[11px] text-muted-foreground" role="status">{message}</span>}</div>{visuals.length > 0 && <div className="grid gap-2 sm:grid-cols-3">{visuals.map((item) => <a key={item.id} href={item.source_url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-md border border-border text-xs hover:bg-muted">{item.image_url && <img src={item.image_url} alt="" className="h-20 w-full object-cover" loading="lazy" referrerPolicy="no-referrer" />}<span className="block p-2">{item.title}<span className="mt-1 block text-muted-foreground">{item.publisher || "Source"}</span></span></a>)}</div>}</div>
}
